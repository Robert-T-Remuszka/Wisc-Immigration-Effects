...
if `cleanperiod1' {
    ...
    forv yyyy = `startyear'(10)1930 {

        if `yyyy' != 1890 { // 1890 Census records were lost in a fire

            use "${Ipums}/Decadal/Acs`yyyy'.dta", clear

            * Drop some of the administrative variables
            drop SAMPLE SERIAL HHWT GQ PERNUM COUNTYNHG 
            if `yyyy' > 1880 drop CLUSTER STRATA
            ...
            * Generate an immigrant indicator
            if `yyyy'  >= 1900 {
                gen imm = year - YRIMMIG < 10
            }
            if `yyyy' < 1900 { // Following Burchardi et al (2019) here
                gen imm = substr(bpl, 1, 1) != "0" | substr(mbpl, 1, 1) != "0" | substr(fbpl, 1, 1) != "0"
            }
